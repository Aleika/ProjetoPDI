...
CascadeClassifier cascadeFace, cascadeSmile;

String cascadeFaceName  = "haarcascades/haarcascade_frontalface_alt.xml";
String cascadeSmileName = "haarcascades/haarcascade_smile.xml";
...
int main( int argc, const char** argv ){
   ...
   if( !cascadeSmile.load( cascadeSmileName )) {
    ...
    String folder = "imagensSalvas/*.jpeg";
...
void detectFace( Mat& img){
  ...
  cascadeFace.detectMultiScale(img, // imagem para deteccao
				   ...
				   Size(20, 20) ); // mínimo tamanho para deteccao de um objeto
  
  for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++ ){
	Mat imgROI;
	vector<Rect> nestedObjects;
	
	if( cascadeSmile.empty() )
	  continue;
